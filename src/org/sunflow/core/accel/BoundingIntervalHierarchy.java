...
public class BoundingIntervalHierarchy implements IntersectionAccelerator {
    ...
    private void buildHierarchy(IntArray tempTree, int[] indices, BuildStats stats) {
        ...
        subdivide(0, objects.length - 1, tempTree, indices, gridBox, 0, 1, stats);
    }
    ...
    private void subdivide(int left, int right, IntArray tempTree, int[] indices, float[] gridBox, int nodeIndex, int depth, BuildStats stats) {
        ...
        float[] prevGridBox = new float[6];
        boolean wasLeft = true;
        float prevClipL, prevClipR;
        while (true) {
            prevAxis = axis;
            prevSplit = split;
            prevClipL = clipL;
            ...
            if (right == rightOrig) {
                // all left
                if (axis == prevAxis && split == prevSplit) {
                ...
                if (prevAxis != -1) {
                    ...
                    for (int i = 0; i < 6; i++)
                        prevGridBox[i] = gridBox[i];
                    right = wasLeft ? rightOrig : left - 1;
                    clipL = prevClipL;
                    clipR = prevClipR;
                }
                break;
        ...
        // free memory
        prevGridBox = gridBox = null;
        // recurse
        if (nl > 0)
            subdivide(left, right, tempTree, indices, gridBoxL, nextIndex, depth + 1, stats);
        else
            stats.updateLeaf(depth + 1, 0);
        if (nr > 0)
            subdivide(right + 1, rightOrig, tempTree, indices, gridBoxR, nextIndex + 3, depth + 1, stats);
        else
            stats.updateLeaf(depth + 1, 0);
    }
