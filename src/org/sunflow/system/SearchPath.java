            ...
            try {
                ...
                for (String prefix : searchPath)
                    if (prefix.equals(path))
                        return;
                UI.printInfo("[SYS] Adding %s search path: \"%s\"", type, path);
                searchPath.add(path);
            } catch (IOException e) {
                UI.printError("[SYS] Invalid %s search path specification: \"%s\" - %s", type, path, e.getMessage());
            }
        } else
            UI.printError("[SYS] Invalid %s search path specification: \"%s\" - invalid directory", type, path);
    }
